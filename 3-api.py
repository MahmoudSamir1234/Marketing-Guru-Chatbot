...
# Initialize Groq LLM
llm = ChatGroq(max_retries=1, model='llama3-70b-8192', api_key=GROQ_API_KEY, temperature=0, max_tokens=2048)
...
def get_pred(input_dict):
    """
    This function takes a dictionary as input, converts it into a DataFrame,
    encodes the categorical variables using pre-loaded label encoders,
    normalizes the data using the pre-loaded scaler if available, and returns
    the churn prediction along with the confidence score.
...
def predict_from_text(customer_description: CustomerDescription):
    """
    This endpoint accepts a customer description in natural language,
    converts it into structured JSON format using Groq LLM, and predicts
    customer churn using the pre-trained model.

    Args:
        customer_description (CustomerDescription): A pydantic model containing
                                                    the customer description.

    Returns:
        str: A message indicating whether the customer is predicted to churn or not,
